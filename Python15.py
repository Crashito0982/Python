...
BEGIN
    ...
    -- Declaramos las variables a utilizar, replicando el doble filtro de fechas del SP original
    DECLARE @DIAS_ATRAS_CANDIDATOS numeric(3) = -30; -- Rango amplio para buscar eventos de entrega candidatos
    DECLARE @DIAS_ATRAS_FILTRO numeric(3) = -15;     -- Rango estricto para filtrar los eventos finales
    ...
    -- PASO 3: Filtrar solo aquellos cuya última entrega fue en los últimos 15 días.
    SELECT evt.docid, doc.doccti AS nro_cliente
    ...
    WHERE evt.evtfch >= @FechaFiltroDesde; -- Filtro final y estricto de 15 días
    ...
    -- PASO 5: Obtener la cédula más reciente para los clientes encontrados
    ...
    WHERE 
        base.valor IN (SELECT nro_cliente FROM #docids_pendientes_final) -- Usamos la lista final de clientes pendientes
