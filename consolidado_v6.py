# -*- coding: utf-8 -*-
"""
BRITIMP - Consolidador v5
-------------------------------------------------------------------------------
Versión mejorada basada en los scripts individuales para mayor precisión.
Cambios clave v5:
- Corregida la lógica de asignación de parsers para todos los tipos de archivo.
- La validación de extensión de archivo ahora es insensible a mayúsculas/minúsculas.
- Detección de otros bancos ahora es más precisa (busca "CLIENTE: ...").
- Detección de "SIN MOVIMIENTOS" ahora es flexible a espacios extra.
- Mejorada la clasificación de archivos "INV DOLAR" y PDFs genéricos.
- Resumen final en el log ahora es más descriptivo.
-------------------------------------------------------------------------------
"""
...
# Se traen los parsers de los scripts individuales para asegurar la lógica correcta
...
def parse_ec_bultos_atm_xlsx(path: Path, DEBUG: bool = False) -> pd.DataFrame:
    ...
    for sheet_name in xl.sheet_names:
        ...
        for _, row_series in df_raw.iterrows():
            ...
            if "TOTAL" in txt and not any(to_ddmmyyyy(c) for c in row): motivo = None; continue
    ...
    for sheet_name in xl.sheet_names:
        ...
        for _, row_series in df_raw.iterrows():
            ...
            registros.append({
                ...
                "CALIDAD_DEPOSITO": int(parse_numeric(row[4] or 0)), "CJE_DEP": int(parse_numeric(row[5] or 0)),
                "CALIDAD_CANJE": int(parse_numeric(row[6] or 0)), "MONEDA": int(parse_numeric(row[7] or 0)),
                "IMPORTE_TOTAL": float(imp), "ARCHIVO_ORIGEN": path.name,
            })
    return pd.DataFrame(registros)

def parse_ec_efect_atm_xlsx(path: Path, DEBUG: bool = False) -> pd.DataFrame:
    """Parser para 'Estado de Cuenta Efectivo ATM'."""
    # Esta es una implementación simplificada, la lógica de EC_ATM_BRI_XLSX.py es más compleja
    # y se puede integrar aquí si es necesario.
    return parse_ec_efect_xlsx_generic(path, "ATM", DEBUG)

def parse_ec_efect_xlsx_generic(path: Path, clasificacion: str, DEBUG: bool = False) -> pd.DataFrame:
    """Parser genérico para Estados de Cuenta de Efectivo."""
    xl = read_excel_any_version(path)
    if not xl: return pd.DataFrame()
    # Reutiliza la lógica de BCO que es muy similar
    return parse_ec_efect_bco_xlsx(path, DEBUG)
...
def process_file(path: Path, parent_agency_hint: Optional[str] = None, DEBUG: bool = False) -> Tuple[pd.DataFrame, Optional[str], str, bool]:
    ...
    # Refinar tipo para PDFs de inventario genéricos
    if tipo == "INV_BILLETES_UNKNOWN":
        if "ATM" in preview.upper():
            tipo = "INV_BILLETES_ATM"
        else:
            tipo = "INV_BILLETES_BCO" # Asume BCO por defecto
        log_info(f"-> Tipo 'UNKNOWN' refinado a: {tipo}")
    ...
    try:
        ...
        elif tipo in ("INV_BILLETES_ATM", "INV_BILLETES_BCO"):
            if ext == ".pdf":
