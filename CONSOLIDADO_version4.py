# -*- coding: utf-8 -*-
"""
BRITIMP - Consolidador v8
-------------------------------------------------------------------------------
Versión mejorada basada en los scripts individuales para mayor precisión.
Cambios clave v8:
- PARSER (EC BCO XLSX): Corregida la lógica para capturar la columna 'BULTOS',
  que anteriormente quedaba vacía.
...
def parse_ec_efect_atm_xlsx(path: Path, DEBUG: bool = False) -> pd.DataFrame:
    ...
    for sheet_name in xl.sheet_names:
        ...
        for _, row_series in df_raw.iterrows():
            ...
            if not started: continue
            if txt_upper.startswith("TOTAL") and not any(to_ddmmyyyy(c) for c in row): motivo, section = None, None; continue
