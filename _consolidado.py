# -*- coding: utf-8 -*-
"""
BRITIMP - Consolidador único (.py)
-------------------------------------------------------------------------------
Estructura de carpetas esperada (se crean si no existen):
...
- Descubrimiento de archivos en PENDIENTES/<AGENCIA> por extensión (.xlsx/.xls/.pdf).
- Gate de validación de CLIENTE ITAU (texto tolerante: Excel/PDF; filtro negativo otros bancos).
- Detección de "SIN MOVIMIENTOS". Si hay → mover a PROCESADO con sufijo " PROCESADO" y no consolidar.
- Inferencia de AGENCIA desde documento/nombre/carpeta.
- Dispatcher por tokens del nombre → ruta a parser del tipo:
    EC_EFECT_ATM / EC_EFECT_BCO / EC_BULTO_ATM / INV_BILLETES_ATM / INV_BILLETES_BCO
- Armonización y exportación por tipo en CONSOLIDADO/AAAA-MM-DD/*.csv.
  Política: por cada tipo, en la PRIMERA escritura de la corrida se pisa el archivo (delete/truncate).
  Las siguientes escrituras del mismo tipo en la corrida hacen append (header=False).
- Movimientos:
    * ITAU con registros → mover a PROCESADO/<AGENCIA> sin renombrar.
    * ITAU sin movimientos o tipo conocido sin registros → mover a PROCESADO/<AGENCIA> renombrando con " PROCESADO".
    * NO ITAU / Tipo no reconocido → mover a PROCESADO/<AGENCIA> sin renombrar.
- Comentado al final: bloque opcional para hacer append automático de los CSV del día a
  CSV maestros ubicados en una carpeta específica (pandas.concat(..., axis=0)).

Nota importante:
- Colocá este script dentro de la carpeta raíz "BRITIMP" para evitar rutas tipo BRITIMP/BRITIMP.
  Si lo ejecutás desde notebook, se usa Path.cwd() como ROOT.
-------------------------------------------------------------------------------
"""
...
def resolve_root() -> Path:
    """
    ROOT robusto:
    - Si es .py: Path(__file__).parent
    - Si es notebook/interactivo: Path.cwd()
    Ubica/crea las subcarpetas exigidas.
    """
    here = Path(__file__).resolve().parent if "__file__" in globals() else Path.cwd()
    root = here  # asumimos que el script vive dentro de BRITIMP/
    # Si detectamos que estamos un nivel arriba y existe BRITIMP/, usarlo.
...
def parse_numeric(val: Any) -> Optional[float]:
    """Parsea números ES/EN y negativos con paréntesis."""
...
def collapse_spaced_letters(s: str) -> str:
    """'B I L L E T E S' -> 'BILLETES' si son solo letras separadas por espacio."""
    tokens = s.split()
    return "".join(tokens) if tokens and all(len(t)==1 for t in tokens) else s

def unir_letras_separadas(texto: str) -> str:
    """Compacta letras separadas: 'B I L L E T E S' -> 'BILLETES'."""
...
def name_matches(fname: str, required_groups: List[List[str]]) -> bool:
    ...
    [['INV'], ['BILLETE','BILLETES'], ['BCO']] => debe tener INV y BILLETE(S) y BCO.
    """
...
def file_text_preview(path: Path, max_rows_excel: int = 40) -> str:
    """
    Retorna un texto "preview" del archivo:
    - XLSX/XLS: concatena primeras filas de todas las hojas como texto.
    - PDF: extrae texto con PdfReader (si disponible).
    - Otros: bytes en bruto como str fallback.
    """
    p = Path(path)
...
def detect_cliente_itau(path: Path, filename_tokens_hint: Optional[List[List[str]]] = None) -> bool:
    """
    Gate para ITAU:
    - Busca patrones tolerantes: 'CLIENTE: ITAU', 'CLIENTE: BANCO ITAU', 'BANCO ITAU', ventana CLIENTE...ITAU.
    - Filtro negativo de otros bancos.
    - Fallback: si el nombre luce del pipeline (INV/EC/ATM/BANCO/BILLETES/EFECT/BULTO) y NO nombra otros bancos.
    """
...
def dispatch_tipo(fname: str) -> Optional[str]:
    ...
    # INV | Inventario BANCO
    if name_matches(up, [["INV"], ["BILLETE","BILLETES"], ["BCO","BANCO","DOLAR","DÓLAR"]]):
        return "INV_BILLETES_BCO"
    return None

# ------------------------------ PARSERS (portados/compactados) ------------------------------
# Nota: Para EC_* en PDF, por simplicidad se ignora (se moverá como NO RECONOCIDO).

def parse_ec_bultos_atm_xlsx(path: Path, DEBUG: bool=False) -> pd.DataFrame:
    """Parser fila→fila para BULTOS ATM (.xlsx)."""
    try:
    ...
    def parse_sheet(df_raw: pd.DataFrame) -> None:
        ...
        for ridx in range(len(df_raw)):
            ...
            # Captura: fecha -> sucursal -> recibo -> [BGS, MGS, BUSD, MUSD]
...
def parse_inv_billetes_xlsx_atm(path: Path, DEBUG: bool=False) -> pd.DataFrame:
    """Inventario de Billetes ATM (.xlsx)."""
    try:
        xl = pd.ExcelFile(path)
    except Exception:
        xl = pd.ExcelFile(path, engine="openpyxl")
...
def parse_inv_billetes_pdf_common(path: Path, DEBUG: bool=False) -> pd.DataFrame:
    """Inventario de Billetes (ATM o BANCO) vía PDF: extracción simple por texto y regex (6 números por fila)."""
...
def parse_ec_efect_xlsx_generic(path: Path, clasificacion: str, DEBUG: bool=False) -> pd.DataFrame:
    """
    Parser genérico EC_EFECT_* (.xlsx). Estructura parecida a BULTOS ATM pero sin desdoblar BGS/BUSD.
    """
    try:
        xl = pd.ExcelFile(path)
    except Exception:
    ...
    for sh in xl.sheet_names:
        ...
        for ridx in range(len(df_raw)):
            ...
            # fecha -> sucursal -> recibo -> (posibles columnas: bultos?, importe?, moneda?)
            ...
            # resto: números (tomamos último como MONTO)
            while idx < len(row):
                n = parse_numeric(row[idx])
                if n is not None: nums.append(n)
                # chequear token de moneda textual
            ...
            bultos = None
            if len(nums) >= 2:
                # algunos reportes traen BULTOS antes del importe
                bn = nums[-2]
            ...
            registros.append({
                ...
                "MONEDA": moneda_guess,
                "ING_EGR": "IN" if section=="INGRESOS" else "OUT",
                "CLASIFICACION": clasificacion,   # 'ATM' o 'BANCO'
                "FECHA_ARCHIVO": fecha_archivo,
                "MOTIVO_MOVIMIENTO": (motivo or section),
                "AGENCIA": agencia,
                "ARCHIVO_ORIGEN": os.path.basename(path),
...
class ProcessResult(Tuple[pd.DataFrame, str, str]): ...
# df, tipo, agencia_out
...
def process_file(path: Path, parent_agency_hint: Optional[str]=None, DEBUG: bool=False) -> Tuple[pd.DataFrame, Optional[str], str, bool]:
    """
    Retorna (df, tipo, agencia_final, es_itau).
    Aplica gates/contingencias y selecciona parser por 'tipo' detectado.
    """
    fname = path.name
    ...
    try:
        if tipo == "EC_BULTO_ATM" and ext in (".xlsx",".xls"):
            ...
            if ext == ".pdf":
                ...
                # Reutilizamos el mismo parser genérico de ATM para estructura xlsx muy similar
                df = parse_inv_billetes_xlsx_atm(path, DEBUG=DEBUG)
            ...
            # tipos no soportados (por ej. EC_* en PDF) → df vacío, seguirá la regla de movimiento
            df = pd.DataFrame()
    except Exception as e:
        if DEBUG:
            print(f"[WARN] Error al parsear {path}: {e}")
        df = pd.DataFrame()

    # Inyección/ajuste de AGENCIA en df resultante
    if not df.empty:
...
def write_consolidated(tipo: str, df: pd.DataFrame) -> Optional[Path]:
    ...
    if tipo not in RUN_WRITTEN:
        # Pisar archivo del día si existe (delete/truncate)
        if outpath.exists():
            outpath.unlink()
        RUN_WRITTEN.add(tipo)
        mode = "w"
        header = True
    # Orden de columnas estándar por tipo (si la df trae columnas extra, se conservan al final)
    ...
    df[cols].to_csv(outpath, index=False, mode=mode, header=header, encoding="utf-8-sig")
    return outpath
...
def safe_move(src: Path, dst_dir: Path, rename_processed: bool=False) -> Path:
    ...
    if not candidate.exists():
        return shutil.move(str(src), str(candidate)) and candidate  # type: ignore
    # evitar colisión → (1), (2), ...
    i = 1
    while True:
        cand = dst_dir / f"{Path(base).stem} ({i}){Path(base).suffix}"
        if not cand.exists():
            return shutil.move(str(src), str(cand)) and cand  # type: ignore
        i += 1
...
def move_original(path: Path, agencia: str, is_itau: bool, had_records: bool, tipo_conocido: bool) -> None:
    ...
    if is_itau and had_records:
        # ITAU con registros → sin renombrar
        safe_move(path, dst, rename_processed=False)
    elif is_itau and (not had_records) and tipo_conocido:
        # ITAU sin movimientos o tipo conocido sin registros → renombrar con " PROCESADO"
        safe_move(path, dst, rename_processed=True)
    else:
        # NO ITAU o Tipo no reconocido → sin renombrar
        safe_move(path, dst, rename_processed=False)
...
def collect_pending_files() -> List[Tuple[Path, Optional[str]]]:
    """
    Recorre PENDIENTES y subcarpetas de agencia (ASU, CDE, ENC, OVD, CON).
    Retorna lista de (path, parent_agencia_hint).
    """
...
def run(DEBUG: bool=False) -> Dict[str, int]:
    ...
    for path, agencia_hint in collect_pending_files():
...
if __name__ == "__main__":
    ...
    out = run(DEBUG=DEBUG)
    # Prints concisos para auditoría opcional:
...
# ------------------------------------------------------------------------------
# Si querés que los CSV del día se agreguen a unos CSV maestros (acumulados),
# descomentá y ajustá TARGET_BASE. Se hará un append deduplicando filas exactas.
#
# from pathlib import Path
# import pandas as pd
#
