# -*- coding: utf-8 -*-
"""
BRITIMP - Consolidador v6
-------------------------------------------------------------------------------
Versión mejorada basada en los scripts individuales para mayor precisión.
Cambios clave v6:
- Lógica de escritura modificada para SOBREESCRIBIR consolidados diarios en
  lugar de añadir (append).
- Mejorada la detección de tipo de PDF (INV ATM/BANCO/DOLAR) para ser directa
  y evitar el estado 'UNKNOWN'.
- Implementados parsers de PDF separados y específicos para ATM y BANCO,
  basados en la lógica de los scripts originales para máxima precisión.
-------------------------------------------------------------------------------
"""
...
def run(DEBUG: bool = False) -> Dict[str, int]:
    ...
    for tipo, df_list in all_dataframes.items():
        if df_list:
            ...
            final_consolidated[tipo] = final_df
            stats[tipo] = len(final_df)

    write_all_consolidated(final_consolidated)
