# -*- coding: utf-8 -*-
"""
BRITIMP - Consolidador v7
-------------------------------------------------------------------------------
Versión mejorada basada en los scripts individuales para mayor precisión.
Cambios clave v7:
- POST-PROCESAMIENTO: Renombrar 'FECHA_OPERACION' a 'FECHA_RECIBO'.
- POST-PROCESAMIENTO: Formatear columnas numéricas como enteros (sin '.0').
- POST-PROCESAMIENTO: Eliminar columna 'ARCHIVO_ORIGEN' de todos los reportes.
- PARSER (INV XLSX): Corregida la asignación de CJE_DEP a CALIDAD_CD.
- PARSER (EC ATM XLSX): Implementado parser específico para evitar datos en blanco.
-------------------------------------------------------------------------------
"""
...
def parse_ec_efect_atm_xlsx(path: Path, DEBUG: bool = False) -> pd.DataFrame:
    """Parser específico para 'Estado de Cuenta Efectivo ATM' (.xls y .xlsx)."""
    xl = read_excel_any_version(path)
    if not xl: return pd.DataFrame()
    registros: List[dict] = []
    ...
    for sheet_name in xl.sheet_names:
        ...
        # Búsqueda más flexible de la fecha del archivo
        ...
        for _, row_series in df_raw.iterrows():
            ...
            if "EGRESOS" in txt: section, motivo, started = "EGRESOS", None, True; continue
            if not started: continue
            if txt.startswith("TOTAL") and not any(to_ddmmyyyy(c) for c in row): motivo = None; continue
            ...
            if nums:
                # Si el primer número es un entero pequeño, es probable que sea Bultos
                if nums[0] == int(nums[0]) and nums[0] < 1000:
                    bultos = nums.pop(0)

            if len(nums) == 2:
                monto_gs, monto_usd = nums[0], nums[1]
            elif len(nums) == 1:
                monto_gs = nums[0]
